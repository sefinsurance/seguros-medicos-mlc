...
Deno.serve(async (req) => {
  try {
    ...
    const { adminToken, phone, campaignName, action } = body;
    ...
    // Sync campaign tag to Postgres
    const response = await fetch(`${baseUrl}/api/prospects/campaign-tag`, {
      method: "POST",
      headers: {
        "x-api-key": apiKey,
        "Content-Type": "application/json",
      },
      body: JSON.stringify({ phone, campaignName, action }),
    });
