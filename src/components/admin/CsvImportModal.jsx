...
export default function CsvImportModal({ campaignId, onClose, onSaved }) {
    ...
    Papa.parse(file, {
      ...
      complete: (results) => {
        const rows = (results.data || []).map((row) => ({
          ...
          birthday: String(row.Birthdate || row.birthdate || row.Birthday || row.birthday || "").trim(),
  ...
  const handleSave = async () => {
    if (!csvRecipients.length) return;
    setSaving(true);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={onClose}>
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
          <h2 className="text-base font-bold text-gray-800">Import Recipients CSV</h2>
          <button onClick={onClose} className="text-gray-400 hover:text-gray-600"><X className="w-5 h-5" /></button>
        </div>
        <div className="px-6 py-5 space-y-4">
          <div className="bg-blue-50 border border-blue-200 rounded-lg px-3 py-2.5 text-xs text-blue-800">
            <strong>CSV Format:</strong> Name, Birthdate (MM/DD/YYYY), Phone, Email. Quoted commas are supported.
          </div>
          <div className="flex gap-2">
            <a href="data:text/csv;charset=utf-8,Name,Birthdate,Phone,Email%0AJohn Doe,01/15/1980,+1234567890,john@example.com" download="sample-recipients.csv" className="flex items-center gap-2 text-xs bg-blue-100 text-blue-700 border border-blue-300 px-3 py-1.5 rounded font-medium hover:bg-blue-200 transition-colors"><Download className="w-3.5 h-3.5" />Sample CSV</a>
          </div>
              ...
              {preview && (
                <div className="border rounded-lg overflow-auto max-h-48 text-xs">
                  <table className="w-full">
                    <thead className="bg-gray-50 sticky top-0"><tr><th className="px-3 py-2 text-left text-gray-500 font-medium">Name</th><th className="px-3 py-2 text-left text-gray-500 font-medium">Birthdate</th><th className="px-3 py-2 text-left text-gray-500 font-medium">Phone</th><th className="px-3 py-2 text-left text-gray-500 font-medium">Email</th></tr></thead>
                    <tbody>
                      {csvRecipients.slice(0, 10).map((r, i) => (
                        <tr key={i} className="border-t border-gray-100"><td className="px-3 py-2 text-gray-700">{r.name}</td><td className="px-3 py-2 text-gray-400">{r.birthday || "—"}</td><td className="px-3 py-2 text-gray-700">{r.phone || "—"}</td><td className="px-3 py-2 text-gray-700">{r.email || "—"}</td></tr>
                      ))}
                    </tbody>
                  </table>
                </div>
