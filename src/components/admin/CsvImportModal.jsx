    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={onClose}>
      ...
      >
                  ...
                  <table className="w-full">
                    <thead className="bg-gray-50 sticky top-0">
                      <tr>
                        ...
                        <th className="px-3 py-2 text-left text-gray-500 font-medium">Birthdate</th>
                    ...
                    <tbody>
                      {csvRecipients.slice(0, 10).map((r, i) => (
                        <tr key={i} className="border-t border-gray-100">
                          ...
                          <td className="px-3 py-2 text-gray-400">{r.birthday || "—"}</td>
                          <td className="px-3 py-2 text-gray-700">{r.phone || "—"}</td>
                          <td className="px-3 py-2 text-gray-700">{r.email || "—"}</td>
                        </tr>
                      ))}
                      {csvRecipients.length > 10 && (
                        <tr><td colSpan={4} className="px-3 py-2 text-gray-400 text-center">...and {csvRecipients.length - 10} more</td></tr>
                      )}
                    </tbody>
                  </table>
                </div>
        ...
        <div className="px-6 py-4 border-t border-gray-100 flex items-center justify-between gap-3">
          ...
          <button
            onClick={handleSave}
            disabled={saving || csvRecipients.length === 0}
