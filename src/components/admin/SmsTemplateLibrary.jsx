...
export default function SmsTemplateLibrary({ onSelectTemplate, onClose }) {
  ...
  const loadTemplates = async () => {
    setLoading(true);
    const data = await base44.entities.SmsTemplate.list("-created_date", 100);
    setTemplates(data);
    setLoading(false);
  };
  ...
  const handleSave = async () => {
    ...
    if (editingId) {
      await base44.entities.SmsTemplate.update(editingId, formData);
    } else {
      await base44.entities.SmsTemplate.create(formData);
    }
