...
function SmsTermsModal({ onClose }) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={onClose}>
      <div
        className="bg-white rounded-2xl shadow-2xl max-w-lg w-full max-h-[80vh] overflow-y-auto p-6 relative"
      ...
      >
        ...
        <div className="text-sm text-gray-700 space-y-3 leading-relaxed">
          <p>
            By opting in to receive SMS communications from MLC Insurance Agency, you agree to receive
            text messages regarding your insurance inquiries, quotes, applications, policy updates,
            reminders, and customer care communications.
          </p>
          ...
          <p>
            ...
            any message. For assistance, reply <strong>HELP</strong> or contact MLC Insurance Agency at{" "}
            <a href="tel:8774582557" className="text-blue-600 hover:underline">
              877-458-2557
            </a>{" "}
            or{" "}
            <a href="mailto:info@mlcinsuranceagency.com" className="text-blue-600 hover:underline">
              info@mlcinsuranceagency.com
            </a>.
          </p>
          <p>
            Your mobile opt-in information will not be shared, sold, or disclosed to third parties for
            marketing purposes. Consent to receive SMS messages is not required as a condition of
            purchasing any goods or services.
          </p>
        </div>
        <button
          onClick={onClose}
          className="mt-5 w-full bg-[#1e3a5f] text-white rounded-lg py-2 text-sm font-semibold hover:bg-[#163059] transition-colors"
        >
          Close
        </button>
      </div>
...
const copy = {
  en: {
    ...
    smsBtn: "Subscribe",
    smsConsent:
      "By subscribing, you consent to receive SMS messages from MLC Insurance regarding health insurance updates, open enrollment reminders, and policy news. Message & data rates may apply. Reply STOP to unsubscribe at any time.",
    smsSuccess: "You're subscribed! We'll keep you updated.",
    smsFailed: "Could not subscribe right now. Please try again.",
    rights: "All rights reserved.",
    disclaimer:
      "MLC Insurance is an independent insurance agency. We are not affiliated with or endorsed by the U.S. government or the federal Medicare program.",
  },
  es: {
    ...
    smsBtn: "Suscribirme",
    smsConsent:
      "Al suscribirte, consientes recibir mensajes SMS de MLC Insurance sobre actualizaciones de seguros, recordatorios de inscripción abierta y noticias de pólizas. Pueden aplicar tarifas de mensajes y datos. Responde STOP para darte de baja en cualquier momento.",
    smsSuccess: "¡Estás suscrito! Te mantendremos informado.",
    smsFailed: "No se pudo completar la suscripción. Inténtalo otra vez.",
    rights: "Todos los derechos reservados.",
    disclaimer:
      "MLC Insurance es una agencia de seguros independiente. No estamos afiliados ni respaldados por el gobierno de EE.UU. ni por el programa federal de Medicare.",
  },
};
...
export default function Footer({ lang = "en" }) {
  ...
  const handleSubscribe = async (e) => {
    ...
    try {
      const response = await fetch("https://mlcagency.base44.app/functions/webhookSmsSubscribe", {
        method: "POST",
        headers: {
      ...
      if (!response.ok) {
        throw new Error(`SMS subscribe failed: ${response.status}`);
      }
    ...
    <footer className="bg-[#1e3a5f] text-white mt-16">
      ...
      <div className="max-w-6xl mx-auto px-4 py-12 grid grid-cols-1 md:grid-cols-4 gap-10">
        <div className="md:col-span-1">
          ...
          <div className="flex flex-col gap-2 mt-4">
            ...
            <a
              href="mailto:info@mlcinsuranceagency.com"
              className="flex items-center gap-2 text-sm text-blue-100 hover:text-white transition-colors"
            >
          ...
          <div className="flex items-center gap-3 mt-4">
            <a
              href="https://www.facebook.com/profile.php?id=100093903970209"
              target="_blank"
              rel="noopener noreferrer"
            ...
            <a
              href="https://www.instagram.com/saludesfortuna/"
              target="_blank"
              rel="noopener noreferrer"
              className="text-blue-200 hover:text-white transition-colors"
            ...
            <a
              href="https://www.tiktok.com/@saludesfortuna"
              target="_blank"
              rel="noopener noreferrer"
              className="text-blue-200 hover:text-white transition-colors"
              aria-label="TikTok"
        ...
        <div>
          ...
          <ul className="space-y-2 text-sm text-blue-100">
            ...
            <li>
              <Link to={createPageUrl("Obamacare")} className="hover:text-white transition-colors">
                ACA / Obamacare
              </Link>
            </li>
            <li>
              <Link to={createPageUrl("MedicareAdvantage")} className="hover:text-white transition-colors">
                Medicare Advantage
              </Link>
            </li>
            <li>
              <Link to={createPageUrl("LifeInsurance")} className="hover:text-white transition-colors">
                Life Insurance
              </Link>
            </li>
            <li>
              <Link to={createPageUrl("DentalVision")} className="hover:text-white transition-colors">
                Dental & Vision
              </Link>
            </li>
            <li>
              <Link to={createPageUrl("ForBrokers")} className="hover:text-white transition-colors">
                {lang === "es" ? "Para Agentes" : "For Brokers"}
              </Link>
            </li>
          </ul>
        </div>
      ...
      <div className="border-t border-white/10">
        <div className="max-w-6xl mx-auto px-4 py-4 flex flex-col md:flex-row items-center justify-between gap-2 text-xs text-blue-300">
          <span>
            © {new Date().getFullYear()} MLC Insurance. {c.rights}
          </span>
