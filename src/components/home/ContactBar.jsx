...
export default function ContactBar({ lang, setLang }) {
  return (
    <header className="bg-[#035582] shadow-sm sticky top-0 z-50">
      <div className="text-slate-50 mx-auto px-4 py-3 max-w-6xl flex items-center justify-between flex-wrap gap-2">
        {/* Logo */}
        ...
        {/* Nav */}
        <nav className="hidden md:flex items-center gap-6 text-sm font-medium text-[#1e3a5f]"></nav>

        {/* Actions */}
        <div className="flex items-center gap-2">
          <button
            onClick={() => setLang(lang === "en" ? "es" : "en")}
            className="bg-slate-50 text-[#1e3a5f] px-3 py-1 text-xs font-semibold rounded-full border border-[#1e3a5f] hover:bg-[#1e3a5f] hover:text-white transition-colors"
          >
            {lang === "en" ? "ES" : "EN"}
          </button>
